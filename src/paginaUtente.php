...
try {
    // Recupero dati utente usando $_SESSION["username"]
    $username = $_SESSION["username"];
    $sql_utente = "SELECT id_utente, nome, cognome, email, data_registrazione 
                FROM utente 
                WHERE email = ?";
    ...
    // Recupero ordini recenti dell’utente
    $sql_ordini = "SELECT id_ordine, data_ordine, stato_ord, totale 
                FROM ordine 
                WHERE id_utente = ? 
                ORDER BY data_ordine DESC LIMIT 5";
    $stmt_ordini = mysqli_prepare($conn_ordini, $sql_ordini);
    mysqli_stmt_bind_param($stmt_ordini, "i", $utente['id_utente']);
    mysqli_stmt_execute($stmt_ordini);
    $result_ordini = mysqli_stmt_get_result($stmt_ordini);
...
<head>
    ...
    <meta name="description"
        content="Gestisci il tuo account personale nel nostro negozio di tè, infusi e tisane di qualità" />
        ...
        <div class="header-container">
            ...
            <div class="header-utilities">
                <button class="icon-button" aria-label="Cerca prodotti">
                    <!--icona di ricerca-->
                    <svg class="icon-svg" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 513.749 513.749"
                        aria-hidden="true">
                        <path
    ...
    <main id="content">
        ...
        <section id="area-account">
            ...
            <section id="ordini-recenti">
                ...
                <?php if (mysqli_num_rows($result_ordini) > 0): ?>
                    <?php while ($ordine = mysqli_fetch_assoc($result_ordini)): ?>
                        <article class="ordine">
                            ...
                            <p><a href="dettaglio-ordine.php?id=<?= $ordine['id_ordine'] ?>">Visualizza dettagli ordine</a></p>
                        </article>
                    <?php endwhile; ?>
                <?php else: ?>
